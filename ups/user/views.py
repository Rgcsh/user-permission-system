...
from ups.service.decorator import request_info, v_login, v_user_role
...
@user_blueprint.route('/acticle', methods=['POST'])
@request_info
@v_login
@v_user_role
def add_acticle():
...
@user_blueprint.route('/acticle', methods=['PUT'])
@request_info
@v_login
@v_user_role
def update_acticle():
	...
	if not Acticle.exist([Acticle.uid == user.id, Acticle.id == acticle_id]):
